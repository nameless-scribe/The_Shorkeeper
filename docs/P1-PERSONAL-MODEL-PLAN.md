...
**状态：已实现并通过逻辑测试及 Electron renderer UI 冒烟（固定模拟数据）。** 同 key 的不同事实会进入候选，原事实转为 `disputed`；用户可保留、替代或并存。替代、来源写入和候选确认处于同一事务，手工编辑也保留历史版本。凭据类内容直接拒绝，敏感和过期事实不会进入模型检索。
...
**状态：已实现并通过自动化及 Electron renderer UI 冒烟（固定模拟数据）。** migration `0024`、预算后来源记录、四因子记忆排序、稳定 `mem/doc/goal/commitment` 引用、回答来源卡片与运行详情回链均已接入；审计只保存限长摘要。
...
**状态：已实现并通过自动化及 Electron renderer UI 冒烟（固定模拟数据）。** migration `0025`、手工检查、显式自动同步、启动/唤醒低频调度、changed/missing 状态、重新定位与成功后版本切换均已接入；旧快照在失败时保持可用。

**目的**：把本地知识库做成可判断新旧的来源；云盘、邮件等外部连接已从当前产品路线取消。
...
**状态：工程验收已完成；真实使用观察窗口已具备开始条件。** `test:p1`、Electron renderer P1 UI smoke（固定模拟数据）、跨里程碑与全量回归作为本次交付证据；这些不等同于真实用户数据库端到端验收。一周观察结果必须在实际使用满周期后补录，当前不虚报为已发生。
...
- `pnpm test:p1:ui`：在真实 Electron renderer 中使用固定模拟数据验证记忆筛选、冲突确认、来源详情、过期状态、知识更新和运行引用；不读取用户数据库，也不作为真实数据端到端证据。
...
- 不接入邮箱、外部日历、联系人或云盘 OAuth；这些能力已从当前产品路线取消。
...
**P1 工程阶段已结束。** 非阻塞的一周真实使用观察继续记录错误引用、冲突漏报、过期误用、候选打扰率与引用帮助度；下一阶段可进入 P3 本地主动服务，发现 P1 问题仍按本契约回补。
