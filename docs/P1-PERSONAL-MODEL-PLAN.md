...
> 版本：0.2.0（P1.0 + P1.1 已落地，等待进入 P1.2）  
> 日期：2026-09-13  
...
当前进度（2026-09-13）：P1.0 契约样本与 P1.1 数据底座已实现。`0023_personal_memory_model.sql`、共享类型、长期记忆/候选/来源 Repository 与双 adapter 临时库测试均已加入；现有提取规则、上下文注入和设置页交互尚未切换到新行为，属于后续 P1.2—P1.5 范围。
...
第一轮 **P1.0 + P1.1 已完成**：固定样本和 `0023` 兼容迁移保持当前模型注入行为不变。下一轮进入 P1.2 冲突闭环，将七类事实接入提取、把替代操作做成事务化版本链，并补齐敏感/过期内容的检索与注入过滤。
