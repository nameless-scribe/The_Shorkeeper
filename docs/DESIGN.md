...
Agent 单段工作请求默认由 10 调整为 20，并增加累计工具槽位、主动耗时、token 与每请求完整输入检查。达到边界后独立无工具收尾，失败时仍输出宿主阶段摘要；阶段结果保存到会话，运行仍标为未完成的 error，terminalReason 区分预算耗尽、缺少必要回答、重复失败。`ask_user` 独立等待 10 分钟，未回答时不执行同批依赖动作。界面保留受控停止的阶段摘要。

第二批已接入聊天预算停止后的检查点：仅在可靠收尾、系统加密可用时保存；运行详情提供“继续一段”，展示累计资源与新增额度，复用现有 IPC、会话锁和权限链。继续前校验来源与文件，事务认领检查点，新 run 保留原目标与回答并关联原任务；已成功的相同非幂等操作禁止重放，续跑副作用重新审批。普通发送“继续”仍是新任务。原记录保持 `error / budget_exhausted` 兼容表示未完成，未新增正式 paused 状态。自动续段、语义进展及领域完整性仍未完成，详见 [执行预算方案](AGENT-EXECUTION-BUDGET-PLAN.md#11-第二批明确授权的检查点续跑2026-09-18)。
