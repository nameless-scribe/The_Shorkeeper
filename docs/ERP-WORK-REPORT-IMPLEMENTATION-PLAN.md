...
日期：2026-09-20。状态：**实施中。M0–M3 的代码与模拟验证已完成首轮，M4/M5 已实现自由对话 Skill、算术验证码自动登录和结果未知回查；真实 ERP 登录与提交仍未验收。**

需求依据：[ERP 对话报工约定](./ERP-WORK-REPORT.md)。完成标准：[开发契约](./DEVELOPMENT-CONTRACT.md)。本计划采用用户已确认的自由表达、允许小数工时、本人当天合计最多 8 小时、确认后通过浏览器报工的要求。

## 实施进度（2026-09-21）

- M0：已锁定生产依赖 `playwright-core@1.63.0`，完成 Vite external、electron-builder files/asar 配置；本机 Edge 153 的可见浏览器模拟探针已验证独立 profile、登录、任务行、双弹窗、0.5 小时填值回读、取消和关闭。打包态模拟探针仍待执行。
...
- M4/M5：自由对话 Skill、同草稿修订、受限算术验证码解析、三次识别上限、`reconcile_erp_report` 结果未知回查已实现首轮。验证码 20 样本、重启/部分完成接续、运行历史业务明细及打包态完整模拟仍待收口。
- 当前自动化：ERP 专项 41 项通过；权限、工具契约、Skill、迁移相关扩大回归 45 项通过；全量测试 225 个文件、1342 项全部通过；`pnpm typecheck` 与生产构建通过。设置页此前已完成默认/紫色/最小窗口 Electron 视觉检查；本次新增确认预览 smoke 在电脑重启后被本机 Electron GPU 子进程 `0xc0000135` 阻断，同一环境下既有 P0 smoke 也在加载 renderer 前失败，尚不能记录为本次视觉通过。
...
- 浏览器运行时和生命周期运行相关 Electron smoke，并新增打包态 ERP 模拟验收脚本。`playwright-core` 仅主进程 external；打包携带所需资源但不携带 profile、验证码、凭据、HAR 或用户数据。
