...
- 2026-09-23 跨模块复审：进入网页最终提交调用后，即使浏览器点击调用抛错也保守标为结果未知，避免误判未写入后重复报工；草稿、接续批次和未知结果回查均核对当前登录站点，旧站点草稿不能在新站点继续修订。新增原失败路径回归测试，ERP 专项 57 项、全量 Vitest 226 个文件 1359 项、类型检查及生产构建通过。Electron 隔离模拟界面在默认/紫色主题和 360×520 最小窗口完成截图检查；真实 ERP 验收仍由用户执行。全量测试首次运行有 1 项非 ERP 导出用例超时，单独重跑及第二次全量运行均通过。
- 2026-09-23 打包态业务模拟扩展：`test:erp:packaged` 使用打包程序的 Playwright 和生产报工服务，在 `127.0.0.1` 模拟 ERP 与临时数据库中验证取消零写入、审批未通过零写入、获批后网页新增一条、远端记录唯一回查及本地账本 `verified`。这覆盖生产业务代码，但审批决定由测试直接写入仓库，尚未覆盖打包应用内的用户确认界面和 Agent 调用。首次隔离打包在 PNG 转图标时因 WebAssembly 内存分配失败；改为准备阶段生成多尺寸 ICO 后，`test:package-icon`、完整隔离打包及新包业务模拟均通过。真实 ERP 和验证码样本仍由用户最终验收。
- 2026-09-23 全局复核：修复 ERP 设置后续字段校验或密码保护失败时前面字段已写入的问题，改为写入前校验并以事务保存；无效 IPC 设置不再先断开当前浏览器。修复 ERP Electron 冒烟测试出错却返回成功码的问题，并加入失败路径验证。ERP 专项 59 项、全量 Vitest 227 个文件 1361 项、类型检查、生产构建、打包图标、Electron runtime 和 100 次窗口生命周期检查均通过；当前隔离打包程序的模拟审批、报工及回查通过。真实 ERP 登录、验证码样本和应用内 Agent 全链路仍由用户验收。
