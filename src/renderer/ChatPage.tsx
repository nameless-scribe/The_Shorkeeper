...
export function ChatPage() {
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden rounded-3xl border border-white/15 bg-gradient-to-b from-shore-bg/95 to-shore-panel/95 shadow-2xl shadow-black/40 backdrop-blur-xl">
      <TitleBar status={status} />
      <MessageList messages={messages} />
      {error && (
        <div className="mx-4 mb-2 rounded-xl border border-red-400/30 bg-red-500/10 px-3 py-2 text-xs text-red-200 no-drag">
          {error}
        </div>
      )}
