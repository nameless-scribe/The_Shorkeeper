...
7. 只有用户明确要求提交已经整理好的草稿时才调用 `submit_erp_report`。该工具会先生成最终业务预览；用户在可信确认弹窗批准后才逐条新增，并以回查到唯一新记录作为完成证据。用户拒绝、预览过期、审批或账本写入失败时不得重试或改用通用点击。
8. `submit_erp_report` 返回 `outcome_unknown` 和 `batchId` 时，调用 `reconcile_erp_report` 只读回查；不能自动再次提交。只有提交或恢复工具返回成功并说明已回查核验，才能告诉用户报工完成。
